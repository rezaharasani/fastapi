...
def get_posts(
    ...
    response = (
        ...
        .filter(
            models.Post.title.icontains(search).__or__(
...
@router.get("/latest", status_code=status.HTTP_200_OK, response_model=schemas.Post)
...
@router.get(
    "/{post_id}", status_code=status.HTTP_200_OK, response_model=schemas.PostOut
)
def get_post_by_id(
    post_id: int,
    db: Session = Depends(get_db),
